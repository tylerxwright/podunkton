<?php
	...
	if($session->user AND $text != "") {
		$result = $database->db_query("INSERT INTO Toons_Comments(toonID, userID, comment, dateSubmitted) VALUES($toonID, $userID, '$text', NOW())");
		$result = $database->db_query("SELECT u.name, u.userID, u.sex, tc.dateSubmitted, tc.comment FROM Users as u JOIN Toons_Comments as tc ON u.userID = tc.userID WHERE tc.toonID = $toonID ORDER BY tc.dateSubmitted DESC LIMIT 0, ".COMMENTS_PER_PAGE);
