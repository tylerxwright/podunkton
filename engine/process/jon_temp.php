<?php
	...
	//swf name and path
	$swf_name = "$safeName_$SwfCounter.swf";
	...
	$tmp_path = "tmp/";
	$tmp_path .= basename($_FILES['iconS']['name']);
	if(move_uploaded_file($_FILES['iconS']['tmp_name'], $tmp_path)) {
		rename($tmp_path, $si_path);
		chmod($si_path, 0777);
	}
	...
	$tmp_path = "tmp/";
	$tmp_path .= basename($_FILES['iconL']['name']);
	if(move_uploaded_file($_FILES['iconL']['tmp_name'], $tmp_path)) {
		rename($tmp_path, $li_path);
		chmod($li_path, 0777);
	}

	//featured image icon name and path
	$fi_name = "featured_icon_$ImageCounter.png";
	$fi_path = "content/toons/image/".$fi_name;

	$tmp_path = "tmp/";
	$tmp_path .= basename($_FILES['fImage']['name']);
	if(move_uploaded_file($_FILES['fImage']['tmp_name'], $tmp_path)) {
		rename($tmp_path, $fi_path);
		chmod($fi_path, 0777);
	}		

	//upload information to database
	$result = $database->db_query("INSERT INTO Toons (name, season, description, releaseDate, ngLink, episode, pointCost, featured, file, safeName, iconSmall, iconLarge, featuredImage) VALUES ('$name', '$season', '$desc', '$release', '$NGLink', '$episode', '$point', '$feature', '$_FILES['file']['name']', '$safeName', '$si_name', '$li_name', '$fi_name')");
