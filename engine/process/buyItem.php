<?php
	...
	if($user) {
		...
		$result = $database->db_query("SELECT munniez FROM Users WHERE userID=$user");
		$row = mysqli_fetch_object($result);
		
		if($row->munniez >= $munniez){
			
			$newMunniez = $row->munniez - $munniez;
			...
			$update = $database->db_query("UPDATE Users SET munniez=$newMunniez WHERE userID = $user");
			
			$sysmsg->send($user, "System: Item Purchase", "You purchased <b>".$rowItem->name."</b> for <b>".$rowItem->munniez."</b> munniez");
