<?php
	...
	$userID = $core->args[2];
	$commenterID = $session->user;
	$text = $purifier->purify($core->args[3]);
	
	$return = "";
	
	if($session->user AND $text != "") {
		$result = $database->db_query("INSERT INTO Users_Comments(userID, commenterID, comment, dateSubmitted) VALUES($userID, $commenterID, '$text', NOW())");
		$result = $database->db_query("SELECT u.name, u.userID, u.sex, uc.dateSubmitted, uc.comment FROM Users as u JOIN Users_Comments as uc ON u.userID = uc.commenterID WHERE uc.userID = $userID ORDER BY uc.dateSubmitted DESC LIMIT 0, ".COMMENTS_PER_PAGE);
