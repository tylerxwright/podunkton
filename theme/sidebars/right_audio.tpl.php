<div id="right_sidebar" style="overflow: hidden;">
	<div class="site_box" style="width: 120px; margin-top: 30px;">
		<div class="site_box_head">
			<img src="/theme/images/site_box/wdck_head.png" />
		</div>
		<a href="/audio/wdck" style="cursor: pointer;">
			<div class="site_box_content" style="text-align: center; padding-top: 4px; padding-bottom: 4px;">
				<img src="/theme/images/wdck.png" style="behavior: url(/iepngfix.htc);" /><br/>
				<span class="wdck">NEW PODCAST!!!</span><br/>
				<span id="small_grey">Next Show: 04-07-08</span>
			</div>
		</a>
	</div>
	<div style="width: 120px; height: 8px; line-height: 1px; font-size: 1px;"></div>
	<div class="site_box" style="width: 120px;">
		<div class="site_box_head">
			<img src="/theme/images/site_box/head_donate.png" />
		</div>
		<div class="site_box_content">
			<div id="donate_container">
				
				<form action="https://www.paypal.com/cgi-bin/webscr" method="post" style="margin: 0px; padding: 0px;">
								<input type="hidden" name="cmd" value="_s-xclick">
								<input type="image" src="/theme/images/paypal_logo.png" style="behavior: url(/iepngfix.htc); border="0" name="submit" alt="PayPal - The safer, easier way to pay online!">
								<!--<img alt="" border="0" src="https://www.paypal.com/en_US/i/scr/pixel.gif" width="1" height="1">-->
								<input type="hidden" name="encrypted" value="-----BEGIN PKCS7----------END PKCS7-----
								">
								</form>
								
				Support Your Community!
			</div>
		</div>
	</div>
	<div style="width: 120px; height: 8px; line-height: 1px; font-size: 1px;"></div>
	<div id="right_ad">
		<img src="/theme/images/adsky2.gif" />
	</div>
</div>