<?php
	...
	$user = $session->user;
	$result = $database->db_query("SELECT COUNT(id) as 'count' FROM Sent WHERE sender = $user");
	$rowSent = mysqli_fetch_object($result);
	$sentNum = $rowSent->count;
	
	$result = $database->db_query("SELECT * FROM Inbox as i JOIN Messages as m ON i.mid = m.mid WHERE reciever = $user ORDER BY postdate DESC");
	$numRows = mysqli_num_rows($result);
?>
<div id="mail_head" style="overflow: hidden;">
	<img src="/theme/images/mail_sign.png" />
</div>


<form action="/process/deleteMessages/inbox" method="POST" name="deleteSelected" style="margin: 0px; padding= 0px;">
<div id="mail_top" style="overflow: hidden;">
	<div id="mail_left" style="overflow: hidden;">
		<div class="top" style="padding: 0px;"><a class="blue" href="/mail/compose"><img src="/theme/images/mail_compose_btn.png" /></a></div>
		<div class="top"><a class="blue" href="/mail">INBOX (<?php echo $numRows; ?>)</a></div>
		<div class="top"><a class="blue" href="/mail/sent">SENTBOX (<?php echo $sentNum; ?>)</a></div>
...
<div class="site_box" style="width: 792px;">
	<div class="site_box_head">
		<table border="0">
			<tr>
				...
				<td class="mail_title" width="270">FROM</td>
	...
	<div class="site_box_content" style="background-color: #fff;">
		<div class="site_box_content_inner" style="width: 792px; padding: 0px 0px 5px 0px; /*background-color: #8092BF;*/ min-height: 400px;">
			<?php
				if($numRows > 0) {
					...
					while($row = mysqli_fetch_object($result)) {
						...
						$resultR = $database->db_query("SELECT name FROM Users WHERE userID = ".$row->sender);
			...
			<?php } ?>
					<div style="width: 300px; padding-left: 4px; float: left;"><a class="blue" href="/mail/inbox/<?php echo $row->mid; ?>"><?php echo $prettyprint->smallString($row->subject, 40); ?></a></div>
