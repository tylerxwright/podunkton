<?php
	...
	$mid = $core->args[2];
	$result = $database->db_query("SELECT * FROM Messages as m JOIN Inbox as i ON m.mid = i.mid WHERE i.mid = $mid AND reciever = $user ORDER BY postdate");
	$numRows = mysqli_num_rows($result);
	if($numRows > 0) {
		$row = mysqli_fetch_object($result);
		
		$result2 = $database->db_query("UPDATE Inbox SET isread = 1 WHERE id = ".$row->id);
...
<div id="mail_view_head">
	...
	To: <a class="blue" href="/user/<?php echo $session->username; ?>"><?php echo $session->username; ?></a>
</div>
		...
		<div class="forums_post_right">
			...
			<div class="forums_post_tools">
				<div class="forums_post_tools_left">
					<?php if($session->user != $rowU->userID AND $session->user) { ?>
					<form name="startTrade" action="/process/startTrade" method="POST" style="margin: 0px; padding: 0px;">
						<a href="/mail/compose/<?php echo $rowU->userID; ?>"><img src="/theme/images/icon_pm2.png" title="Private Message" /></a>
						<img style="cursor: pointer;" src="/theme/images/icon_trade2.png" title="Start Trade" onclick="document.startTrade.submit();" />
						<?php if($rowU->aim != "") { ?>
